    ...
    <nav id="header" class="w-full z-30 top-0 py-1">
        <div class="w-full container mx-auto flex flex-wrap items-center justify-between mt-0 px-6 py-3">
            ...
            <div class="order-2 md:order-3 flex items-center" id="nav-content">
                ...
                <style>
                    /* كود لتشغيل القائمة عند الهوفر بدون جافا سكريبت */
                    .dropdown:hover .dropdown-menu {
                        display: block;
                    }

                    /* لمسة فخامة: تنعيم الظهور */
                    .dropdown-menu {
                        display: none;
                ...
                <div class="dropdown relative inline-block">
                    ...
                    <a class="inline-block no-underline hover:text-black p-2" href="#">
                    ...
                    <div
                        class="dropdown-menu absolute right-0 w-48 bg-white border border-gray-100 shadow-xl rounded-xl z-50 overflow-hidden">
                        <div class="py-2">
                            ...
                            @auth
                                ...
                                <a href="{{ url('/profile') }}"
                                    class="block px-4 py-1 text-[12px] text-gray-700 hover:bg-black hover:text-white transition-all duration-200">
                                    MY ACCOUNT
                                </a>
                                ...
                                @if ($user->role === 'customer')
                                    ...
                                    <a href="{{ route('vendors-requests.create') }}"
                                        class="block px-4 py-1 text-[12px] text-blue-600 hover:bg-gray-100 transition-all duration-200 uppercase font-bold">
                                        Sell on NOVA
                                    </a>
                                @endif
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="w-full text-left block px-4 py-1 text-[12px] text-red-600 hover:bg-red-50 transition-all duration-200 uppercase font-bold">
                                        Logout
                                    </button>
                                </form>
                            @else
    ...
    Sell on NOVA
                ...
                <a class="pl-3 inline-block no-underline hover:text-black relative shake-it"
                    ...
                    @auth
                        ...
                        @if ($totalItems > 0)
                            <span
                                class="absolute -top-1.5 -right-1.5 bg-red-600 text-white text-[9px] w-4 h-4 flex items-center justify-center rounded-full font-bold border border-white shadow-sm">
                                {{ $totalItems }}
                            </span>
                        @endif
                    @endauth
            ...
            gab-6">
                @foreach ($products as $product)
                    <div
                        ...
                        <form action="{{ route('carts.store', $product->variants->first()->id) }}" method="POST">
                            @csrf <button type="submit">Add to cart</button>
                        </form>
                    </div>
                @endforeach
            </div>
